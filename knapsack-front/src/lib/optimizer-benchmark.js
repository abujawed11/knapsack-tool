...
// Version: 1.1 - Fixed "Invalid array length" error with try-catch and validation
...
export function dpOptimized({
  ...
  try {
    ...
    if (!Number.isFinite(R) || R === 0) return { ok:false, reason:"Required length must be a valid positive number." };
    if (R < 0) return { ok:false, reason:"Required length cannot be negative." };

    const maxL = Math.max(...L);
    if (!Number.isFinite(maxL)) return { ok:false, reason:"Invalid cut lengths." };

    // OPTIMIZATION 1: Reduce TMAX with reasonable overshoot limit
    const maxOvershoot = Math.min(maxL - 1, Math.max(100, R * 0.05));
    const TMAX = Math.floor(R + maxOvershoot);
    ...
    if (!Number.isFinite(TMAX) || TMAX < 0 || TMAX > MAX_ARRAY_SIZE) {
      return {
        ok: false,
        reason: `Problem too large for DP Optimized (TMAX=${TMAX}, R=${R}). Try Greedy instead.`
      };
    }
    ...
    // Extra safety check before creating array
    const arraySize = TMAX + 1;
    if (arraySize > MAX_ARRAY_SIZE || arraySize < 0 || !Number.isInteger(arraySize)) {
      return {
        ok: false,
        reason: `Invalid array size (${arraySize}). Problem too large for DP Optimized.`
      };
    }

    const best = new Array(arraySize).fill(null);
    best[0] = { total: 0, pieces: 0, small: 0, prev: null, lastIdx: null };

    // Helper function for DP comparison
  ...
  return {
    ...
    return {
      ok: false,
      reason: `DP Optimized error: ${error.message}`
    };
  }
}
